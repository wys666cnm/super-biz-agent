...
public class ChatController {
    ...
    public ResponseEntity<ApiResponse<ChatResponse>> chat(@RequestBody ChatRequest request) {
        try {
            ...
            // 创建 ReactAgent（使用 DeepSeek ChatModel）
    ...
    public SseEmitter aiOps() {
        ...
        executor.execute(() -> {
            try {
                ...
                // 使用自动配置的 ChatModel（DeepSeek v4 Flash）
                // 注意：AIOps 使用较低温度以保持输出稳定
                logger.info("开始 AIOps 分析流程，DeepSeek ChatModel: {}", chatModel.getClass().getSimpleName());
